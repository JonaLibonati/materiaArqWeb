const body = document.getElementsByClassName("body")[0]

const nav = document.getElementsByClassName("nav")[0]
const nav__list = document.getElementsByClassName("nav__list")[0]
const nav__butList = document.getElementsByClassName("nav__butList")[0]
const nav__curtain = document.getElementsByClassName("nav__curtain")[0]
const nav__blur = document.getElementsByClassName("nav__blur")[0]
const nav__item = document.getElementsByClassName("nav__item")
const nav__itemFirst = document.getElementsByClassName("nav__item--first")[0]

const about = document.getElementsByClassName("about")[0]

const pj = document.getElementsByClassName("pj")[0]
const pj__box = document.getElementsByClassName("pj__box")
const pj__blur = document.getElementsByClassName("pj__blur")
const pj__textBox = document.getElementsByClassName("pj__textBox")
const pj__descrip = document.getElementsByClassName("pj__descrip")
const pj__chevronClose = document.getElementsByClassName("pj__chevronClose")
const pj__chevronOpen = document.getElementsByClassName("pj__chevronOpen")
const pj__textChevron = document.getElementsByClassName("pj__textP--chevron")

const ph = document.getElementsByClassName("ph")[0]
const close_x = document.getElementsByClassName("ph__close")[0]

const pj__chevronPhoto = document.getElementsByClassName("pj__chevronPhoto")[0]